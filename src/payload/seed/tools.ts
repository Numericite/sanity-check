...
export async function seed(payload: Payload) {
    ...
    for (const tool of list) {
        ...
        if (tool.enterprise_location?.length) {
            toolData.locations_enterprise = tool.enterprise_location.map((l: any) => ({
                location: locationMap[l],
            }));
        }

        if (tool.location_host_client?.length) {
            toolData.locations_host_client = tool.location_host_client.map((l: any) => ({
                location: locationMap[l],
            }));
        }

        if (tool.final_users_location?.length) {
            toolData.locations_final_users = tool.final_users_location.map((l: any) => ({
                location: locationMap[l],
            }));
        }
